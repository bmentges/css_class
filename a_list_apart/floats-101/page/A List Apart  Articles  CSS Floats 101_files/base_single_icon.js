...
//                                    spsupport.log("puPSearch " +  superfish.b.inimgSrp + "   " + sp.prodPage.i);
//                                    if (superfish.b.inimgSrp && sp.prodPage.i == 0) {
//                                        sp.SRP.c = (sp.SRP.c > 0 ? sp.SRP.c-1 : 0);
//                                        spsupport.log("puPSearch srp "  );
//                                        spsupport.api.sSrp();
//                                    }
//                                }   
//                                else {
//                                    spsupport.log("puPSearch send request"  );
...
//                                }
    ...
    setLimits: function(ip, pp) {
        var sp = spsupport.p;
        sp.bh.x1 = ip.x;//(Math.min(ip.x, pp.x) - 20);
        sp.bh.y1 = ip.y; //(Math.min(ip.y, pp.y) - 20);        
        sp.bh.x2 = ip.x+ip.w; //(Math.max(ip.x+ip.w, pp.x+sfBg1.width()) + 20);
        sp.bh.y2 = ip.y+ip.h; //(Math.max(ip.y+ip.h, pp.y+bubbleHeight) + 20);
//        spsupport.log("setLimits " + sp.bh.x1 + "  " + sp.bh.y1 + "  " + sp.bh.x2 + "   " + sp.bh.y2);
    },
    ...
    inside: function(e) {
        ...
        var t = e.pageX > sp.bh.x1 && e.pageX < sp.bh.x2 && e.pageY > sp.bh.y1 && e.pageY < sp.bh.y2; //, // || 
            //p = e.pageX > sp.bh.px1 && e.pageX < sp.bh.px2 && e.pageY > sp.bh.py1 && e.pageY < sp.bh.py2;
//        // spsupport.log("x1 " + sp.bh.x1 + "   x2 " + sp.bh.x2 + "  y1 " + sp.bh.y1 + "  y2 " + sp.bh.y2 + " px1 " + sp.bh.px1 + "   px2 " + sp.bh.px2 + "  py1 " + sp.bh.py1 + "  py2 " + sp.bh.py2 + "  =======  x " + e.pageX + "   y " + e.pageY + "      " + t + "   " + p);
         return (t); 
    },    
    ...
    hoverMove : function(e){  
        var sp = spsupport.p,            
            sa = spsupport.api;
//        spsupport.log("mousemove " + sa.inside(e));
...
//                    spsupport.log("top reset to -200");
...
//        });
    ...
    addSFIcon: function(parent){        
        var sp = spsupport.p,
            ...
            nm = superfish.b.redIcon ? 'ri' : (superfish.b.whiteIcon ? 'wi' : 'si'),
    ...
    positionSFDiv: function( img ){
        ...
        var io = (nI.type == 1 ? spi.big : spi.small),
            t = superfish.b.redIcon || superfish.b.whiteIcon ? (imgPos.y + 2) : (img.height > 199 ? (imgPos.y + img.height - io.h + 3) : (imgPos.y + img.height - img.height/6)),
            // show see similar button on left side of image
            align = (img.width > spi.big.w*2 ? (imgPos.x + 1) : (imgPos.x - (io.w - img.width)/2));
        ...
        var l = superfish.b.redIcon || superfish.b.whiteIcon ? (imgPos.x - (io.w - img.width)/2) : align;
